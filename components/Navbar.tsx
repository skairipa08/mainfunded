...
function NavDropdown({ label, icon: Icon, items }: NavDropdownProps) {
  ...
  return (
    <div className="relative" ref={ref}>
      ...
      {open && (
        <div className="absolute top-full left-0 mt-2 w-64 bg-white rounded-xl shadow-xl border border-gray-200 py-2 z-50" role="menu">
          {items.map((item) => (
            <Link
              key={item.href}
              href={item.href}
          ...
          <div className="flex items-center space-x-2 sm:space-x-3">
            ...
            {user && (
              <Link
                ...
                aria-label={t('components.navbar.ba_takvimi')}
                title={t('components.navbar.ba_takvimi')}
              >
                <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect width="18" height="18" x="3" y="4" rx="2" ry="2" /><line x1="16" x2="16" y1="2" y2="6" /><line x1="8" x2="8" y1="2" y2="6" /><line x1="3" x2="21" y1="10" y2="10" /></svg>
                <span className="sr-only">{t('components.navbar.ba_takvimi')}</span>
              </Link>
            )}
