...
function DonatePageContent() {
  ...
  useEffect(() => {
    ...
    if (campaignParam === 'special-needs') {
      setFormData(prev => ({ ...prev, target: 'Special needs children' }));
    } else if (campaignParam === 'education-equality') {
      setFormData(prev => ({ ...prev, target: 'General education fund' }));
    }
  }, [searchParams]);

  // Fetch campaign data if campaign_id is provided, or mock for special stories
  useEffect(() => {
    const storyParam = searchParams.get('story');
    if (storyParam) {
      setCampaignData({
        title: (t('specialNeeds.navLabel') !== 'specialNeeds.navLabel' ? t('specialNeeds.navLabel') : 'Özel Gereksinimli Çocuklar') + ' Destek Fonu',
        student: { name: storyParam.charAt(0).toUpperCase() + storyParam.slice(1).toLowerCase() },
        raised_amount: 0,
        goal_amount: 50000,
        country: 'Türkiye'
      });
                    ...
                    {!campaignData && (
                      <div className="space-y-2">
                        ...
                        <Select
                          ...
                          onValueChange={(value: 'Support a verified student' | 'General education fund' | 'Special needs children') =>
                        ...
                        >
                          ...
                          <SelectContent>
                            ...
                            <SelectItem value="Special needs children">
                              <div className="flex items-center gap-2">
                                <Heart className="h-4 w-4 text-pink-500" />
                                <span>{t('specialNeeds.navLabel') !== 'specialNeeds.navLabel' ? t('specialNeeds.navLabel') : 'Özel Gereksinimli Çocuklar'}</span>
                              </div>
                            </SelectItem>
                          </SelectContent>
                        </Select>
