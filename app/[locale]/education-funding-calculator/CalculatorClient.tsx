...
const PRICES = {
  book:       200,  // ders kitabı — yardımcı kitap/çalışma kitabı
  meal:        80,  // okul öğünü — okul kantini
  day:        150,  // eğitim günü — ulaşım + kırtasiye + yemek
  stationery: 400,  // kırtasiye seti — kalem, defter, cetvel vb.
  tutoring:   350,  // etüt/özel ders saati — grup etüt
}
                ...
                <div key={cat.key} className={`border rounded-2xl p-4 ${cat.color}`}>
                  <div className="flex items-start justify-between mb-2">
                    ...
                    <span className={`text-xs font-semibold px-2 py-0.5 rounded-full ${cat.badge}`}>
                      ₺{fmt(cat.price)}/{isTr ? cat.unit : cat.unit}
                    </span>
                  </div>
                  <p className={`text-3xl font-black mb-0.5 ${cat.num}`}>{fmt(count)}</p>
            ...
            <div className="space-y-2">
              {impact.days >= 180 && (
                <p className="text-sm text-slate-700">
                  ...
                  {isTr ? ` — ${fmt(impact.days)} gün, Türkiye'de bir akademik yıla (${fmt(impact.days >= 180 ? Math.floor(impact.days / 180) : 1)} yıl) karşılık gelir.` : ` — ${fmt(impact.days)} days covers ${Math.floor(impact.days / 180)} full academic year(s) in Turkey.`}
                </p>
              )}
              {impact.books >= 8 && (
                <p className="text-sm text-slate-700">
                  <span className="font-bold text-amber-700">📚 {isTr ? 'Tam set kitap' : 'Full book set'}</span>
                  {isTr ? ` — Bir öğrencinin tüm yıllık ders kitaplarını (ortalama 8 kitap) karşılar.` : ` — Covers a student's entire yearly textbook set (average 8 books).`}
                </p>
              )}
              {impact.meals >= 20 && (
                <p className="text-sm text-slate-700">
                  <span className="font-bold text-emerald-700">🍱 {isTr ? '1 ay öğün' : '1 month of meals'}</span>
                  {isTr ? ` — ${fmt(impact.meals)} öğün, yaklaşık ${Math.floor(impact.meals / 20)} aylık okul öğününe karşılık gelir.` : ` — ${fmt(impact.meals)} meals ≈ ${Math.floor(impact.meals / 20)} month(s) of daily school lunches.`}
                </p>
              )}
              {impact.tutoring >= 4 && (
                <p className="text-sm text-slate-700">
                  <span className="font-bold text-purple-700">🎓 {isTr ? 'Etüt desteği' : 'Tutoring support'}</span>
                  {isTr ? ` — ${fmt(impact.tutoring)} saat etüt, bir öğrencinin sınav hazırlığına ciddi katkı sağlar.` : ` — ${fmt(impact.tutoring)} tutoring hours makes a meaningful difference for exam preparation.`}
                </p>
              )}
