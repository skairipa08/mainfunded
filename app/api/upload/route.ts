...
/**
 * POST /api/upload
 * Public endpoint for application file uploads (documents, photos & videos).
 * Accepts multipart/form-data with `file` and optional `type` (document|photo|video).
...
export async function POST(request: NextRequest) {
  ...
  const rateLimitResponse = withRateLimit(request, RATE_LIMITS.verificationUpload);
  ...
  try {
    ...
    if (content.length > maxSize) {
      return NextResponse.json(
        {
          success: false,
          error: `Dosya boyutu çok büyük. Maksimum ${fileType === 'video' ? '50' : fileType === 'photo' ? '5' : '10'} MB.`,
        },
        { status: 400 }
      );
    }
    ...
    if (config) {
      ...
      uploadFormData.append('file', new Blob([content], { type: file.type }), file.name);
